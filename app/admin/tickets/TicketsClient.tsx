...
/**
 * Founder task board.
 *   - Filter pills by status (with live counts)
 *   - "New task" modal (title, assignee, priority, due date, description)
 *   - "Add teammate" modal (name + phone/email → creates a login)
 *   - Row click expands a detail panel: inline status / assignee /
 *     priority / due controls, a note box, and the activity timeline.
 *
 * All mutations hit /api/admin/* then refresh. Mirrors the UsersClient
 * pattern so the look + interactions match the rest of /admin.
 */
...
export default function TicketsClient({
  ...
  const refreshTeam = useCallback(async () => {
    ...
    if (res.ok)
      setTeam(
        (json.team || []).map((t: TeamMember) => ({ id: t.id, name: t.name })),
      );
  }, []);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      >
        <div>
          ...
          <p style={subtitleStyle}>
            Everything your team is working on. Create a task, assign it, and
      ...
      {migrationNeeded && (
        <Card style={{ borderColor: "rgba(217,119,6,0.35)" }}>
          ...
          <p style={{ fontSize: 13.5, color: "#525252", margin: 0, lineHeight: 1.55 }}>
            The tickets tables aren&apos;t in your database yet. Open the
            Supabase dashboard → SQL Editor, paste the contents of{" "}
            <code style={codeStyle}>supabase/schema.sql</code>, and click Run.
      ...
      <Card padding={0}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr>
                {["Task", "Assignee", "Priority", "Status", "Due", "Updated", ""].map(
                  (h) => (
                ...
                <tr>
                  <td colSpan={7} style={{ padding: 0 }}>
                    <EmptyState
                      ...
                      description={
                        migrationNeeded
                          ? "Once the database is set up, your tasks will show here."
                      ...
                      >
                        ...
                        <td style={tdStyle}>
                          {t.assignee ? (
                            ...
                            <span style={{ color: "#A3A3A3" }}>Unassigned</span>
                          )}
                        </td>
                        <td style={tdStyle}>
                        ...
                        <td style={{ ...tdStyle, fontSize: 13 }}>
                          <DueLabel due={t.due_date} done={t.status === "done"} />
                        </td>
                        <td style={{ ...tdStyle, color: "#525252", fontSize: 13 }}>
                          {relativeTime(t.updated_at)}
                        </td>
                        <td style={tdStyle}>
                          <span style={{ color: "#A3A3A3" }}>{open ? "▾" : "▸"}</span>
                        </td>
                      </tr>
                      {open && (
      ...
      {/* Add teammate modal */}
      <Modal open={teamOpen} onClose={() => setTeamOpen(false)} title="Add teammate">
        <AddTeammateForm
          onCreated={(msg) => {
            setTeamOpen(false);
            setToast({ kind: "success", message: msg });
...
function AiCreateForm({
  ...
  const runDraft = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        onError(json.error || "Couldn't draft tasks.");
        return;
      }
      const d: Draft[] = json.drafts || [];
      if (d.length === 0) {
        onError("No tasks found in that note. Try adding more detail.");
        return;
      }
      setDrafts(d);
    } finally {
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: 16, display: "flex", justifyContent: "flex-end" }}>
          <Button onClick={runDraft} loading={drafting} disabled={!text.trim()}>
            {drafting ? "Reading…" : "✨ Draft tasks"}
          </Button>
        </div>
      </div>
    );
  ...
  // Stage 2 — review + edit drafts
  return (
    <div>
      <p style={{ fontSize: 13, color: "#525252", margin: "0 0 14px" }}>
        Review and tweak, then create. {drafts.length} task
      ...
      <div style={{ marginTop: 16, display: "flex", justifyContent: "space-between", gap: 10 }}>
        <Button variant="secondary" onClick={() => setDrafts(null)} disabled={creating}>
          Back
        </Button>
        <Button onClick={createAll} loading={creating} disabled={drafts.length === 0}>
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {ticket.description ? (
          <p style={{ fontSize: 13.5, color: "#404040", margin: "8px 0 16px", lineHeight: 1.55 }}>
            {ticket.description}
          </p>
        ) : (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <Field label="Assignee">
            <select
              ...
              onChange={(e) => change({ assignee_id: e.target.value || null })}
              style={selectStyle}
            >
              <option value="">Unassigned</option>
              {team.map((m) => (
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            ...
            />
            <Button onClick={postNote} loading={busy} disabled={!note.trim()}>
              Post
            </Button>
          </div>
        </div>
                ...
                <li key={u.id} style={{ display: "flex", gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, color: "#0A0A0A" }}>
                      <ActivityText update={u} />
                    </div>
                    <div style={{ fontSize: 11, color: "#A3A3A3", marginTop: 2 }}>
                      {u.author?.name || "Someone"} · {relativeTime(u.created_at)}
                    </div>
                  </div>
                </li>
              ))}
...
function ActivityText({ update: u }: { update: TicketUpdate }) {
  ...
  if (u.type === "status_change")
    return (
      <>
        Moved from{" "}
        <b>{u.old_status ? STATUS_LABEL[u.old_status] : "—"}</b> to{" "}
        <b>{u.new_status ? STATUS_LABEL[u.new_status] : "—"}</b>.
      </>
    );
  return <>{u.body}</>;
}
...
function NewTaskForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        onError(json.error || "Failed to create task.");
        return;
      }
  ...
  return (
    <form onSubmit={submit}>
      <Input
        ...
        placeholder="e.g. Finish the Dubai order"
        required
        autoFocus
      />
      <div style={{ height: 14 }} />
      ...
      {team.length === 0 && (
        <p style={{ fontSize: 12.5, color: "#A3A3A3", margin: "12px 0 0" }}>
          Tip: add a teammate first so you can assign this task to someone.
        </p>
      )}
...
function AddTeammateForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        onError(json.error || "Failed to add teammate.");
        return;
      }
      onCreated(`Added ${name.trim()} to the team.`);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div style={{ height: 14 }} />
      <Input
        label="Phone (they log in with this)"
        value={phone}
      ...
      <div style={{ height: 14 }} />
      <Input
        label="Email (optional alternative login)"
        type="email"
        value={email}
        onChange={(e) => setEmail(e.target.value)}
        placeholder="name@example.com"
      />
      <p style={{ fontSize: 12.5, color: "#A3A3A3", margin: "12px 0 0", lineHeight: 1.5 }}>
        They sign in at <code style={codeStyle}>/login</code> with a one-time
        code, then see their tasks at <code style={codeStyle}>/my-tasks</code>.
      </p>
      <div style={{ marginTop: 18, display: "flex", justifyContent: "flex-end" }}>
        <Button
          type="submit"
...
function DueLabel({ due, done }: { due: string | null; done: boolean }) {
  ...
  return (
    <span style={{ color: overdue ? "#B91C1C" : "#525252", fontWeight: overdue ? 600 : 400 }}>
  ...
  return (
    <div
      style={{
        padding: "10px 14px",
        background:
          toast.kind === "success" ? "rgba(22,163,74,0.10)" : "rgba(220,38,38,0.08)",
        border: `1px solid ${
