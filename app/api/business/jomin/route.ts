/**
 * POST /api/business/jomin — public demo chat for the Jomi Insurance
 * WhatsApp agent.
 *
 * This powers the /business/jomin sales-demo page: a prospect (or Jomi's
 * own team) can chat with the configured insurance agent before the real
 * WhatsApp integration is wired up. No auth — it's a public demo surface
 * — so we lean on a per-IP rate limit to keep the Anthropic bill sane.
 *
 * The Anthropic call, model fallback, and SSE re-streaming all live in
 * `lib/anthropic-stream.ts` (shared with the student coach). This route
...
import { JOMI_INSURANCE_PROMPT } from "@/lib/jomi-insurance-prompt";
...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`jomin:${ip}`, MAX_PER_WINDOW, WINDOW_SECONDS);
  if (!rl.ok) {
    return new Response(
      JSON.stringify({
        error: "rate-limited",
...
Spoken style: short — 2 to 4 short sentences, one idea, end with one question. Numbers as rounded words, never digit strings (e.g. "around twelve thousand", "ten lakh cover"). No markdown, no bullet symbols, no emoji, no asterisks. It must sound natural read out loud.`;
  ...
  if (!apiKey) {
    console.error("[jomin] ANTHROPIC_API_KEY not set");
