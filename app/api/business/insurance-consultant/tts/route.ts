/**
 * POST /api/business/insurance-consultant/tts — text-to-speech for the
 * Insurance Consultant voice demo. Turns the agent's reply text into a
 * spoken voice note in the matching language (Malayalam or English).
 *
...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`insurance-consultant-tts:${ip}`, MAX_PER_WINDOW, WINDOW_SECONDS);
  if (!rl.ok) {
  ...
  if (!apiKey) {
    console.error("[insurance-consultant-tts] ELEVENLABS_API_KEY not set");
  ...
  try {
    ...
    console.error("[insurance-consultant-tts] upstream fetch failed:", err);
  ...
  if (!upstream.ok || !upstream.body) {
    ...
    console.error(
      "[insurance-consultant-tts] elevenlabs",
      upstream.status,
      "·",
      errText.slice(0, 300),
    );
