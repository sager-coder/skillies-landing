/**
 * POST /api/business/insurance-consultant/transcribe — speech-to-text for
 * the Insurance Consultant voice demo, via OpenAI gpt-4o-transcribe.
 *
 * A caller speaks English, Malayalam, or a mix. gpt-4o-transcribe
 * auto-detects the language and returns the correct NATIVE script
 * (Malayalam Unicode for Malayalam, plain English for English) — verified
 * on real clips. The `prompt` keeps insurance terms and brand names in
 * English even within a Malayalam sentence.
 *
 * Key: reads the GENERAL OPENAI_API_KEY (shared), by design.
 *
 ...
 * the script of the returned text.
 *
...
const PROMPT =
  ...
  "Malayalam (often mixing in English insurance/finance terms). Keep these " +
  "terms and brand names in English even within a Malayalam sentence: " +
  "premium, policy, term plan, ULIP, fund, sum assured, maturity, annuity, " +
  "pension, guaranteed income, money back, nominee, rider, IRR, return, " +
  "SBI Life, Axis Max Life, HDFC Life, ICICI Prudential, Tata AIA.";
...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`insurance-consultant-stt:${ip}`, MAX_PER_WINDOW, WINDOW_SECONDS);
  if (!rl.ok) {
  ...
  if (!apiKey) {
    console.error("[insurance-consultant-stt] OPENAI_API_KEY not set");
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      console.error("[insurance-consultant-stt] openai fetch failed:", err);
    ...
    if ((upstream.status === 429 || upstream.status === 503) && attempt < 2) {
      console.warn(`[insurance-consultant-stt] openai ${upstream.status} — retry ${attempt + 1}`);
      await new Promise((r) => setTimeout(r, 600 * (attempt + 1)));
      continue;
    }
  ...
  if (!upstream || !upstream.ok) {
    ...
    console.error("[insurance-consultant-stt] openai failed", status, "·", errText.slice(0, 300));
