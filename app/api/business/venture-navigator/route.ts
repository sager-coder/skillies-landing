...
Spoken style: write in SHORT, self-contained sentences — each sentence is played as its own spoken voice note, so keep every sentence short and clean (the cloned voice slurs long ones). EARLY in the conversation (the first couple of exchanges) reply in just ONE short sentence — that's how WhatsApp voice chats open. Once the founder is engaged and you genuinely have more to say, you MAY reply in 2 to 4 short sentences (each becomes its own note), so they get a fuller answer without waiting turn-by-turn. You judge how many; default to fewer. Say numbers as rounded Malayalam words, never digit strings (e.g. "ഏകദേശം അമ്പത് ലക്ഷം", "പന്ത്രണ്ട് ശതമാനം equity"). No markdown, no bullet symbols, no emoji, no asterisks. Every sentence must sound natural read out loud.`;
    ...
    if (li >= 0) {
      sanitized.turns[li] = {
        ...
        content:
          ...
          "\n\n[SPOKEN VOICE REPLY — reply ONLY in Malayalam script (മലയാളം), even though I wrote in English. English allowed ONLY for single startup NOUNS (startup, MRR, traction, pre-seed). All VERBS in Malayalam — no 'chase ചെയ്യുന്നു', no two English words in a row (the voice slurs them). Do NOT reply in full English. Short self-contained sentences (each is played as its own voice note) — just ONE short sentence now if we're starting; 2-4 short sentences only once I'm engaged.]",
      };
    }
  }
