 ...
 * agent, with text AND voice.
 *
 * Text: type → /api/business/jomin streams a reply (model fallback).
 * Voice: hold the mic → record → /api/transcribe (Scribe, auto-detects
 *   Malayalam vs English) → /api/business/jomin in voice mode (agent
 *   replies in matching language, TTS-friendly script) → /api/business/
 *   jomin/tts speaks it back → the reply auto-plays as a voice note.
 *
...
const INTRO: Msg = {
  ...
  role: "assistant",
  content:
    "നമസ്കാരം 🙏 Jomi Insurance-ലേക്ക് സ്വാഗതം. ഏത് insurance-നെ കുറിച്ചാണ് അറിയേണ്ടത് — life, health, vehicle, അല്ലെങ്കിൽ home? (Type ചെയ്യാം, അല്ലെങ്കിൽ mic പിടിച്ച് സംസാരിക്കാം.)",
  ts: Date.now(),
};
...
export default function JominChatClient() {
  ...
  const sendRecording = useCallback(async () => {
    ...
    // 2) Transcribe via Gemini (reliable Malayalam/English detection +
    ...
    try {
      ...
      const tr = await fetch("/api/business/jomin/transcribe", {
        method: "POST",
        body: fd,
      });
      if (tr.ok) {
    ...
    let aiDur = 0;
    try {
      const tts = await fetch("/api/business/jomin/tts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={page}>
      ...
      <style>{`
        ...
        @keyframes jpulse { 0%,100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.5; transform: scale(0.8); } }
...
function Dot({ d }: { d: string }) {
  return (
    <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#90a4ae", display: "inline-block", animation: `jdot 1.2s ${d} infinite ease-in-out` }} />
  );
}
...
const recDot: React.CSSProperties = { width: 10, height: 10, borderRadius: "50%", background: "#d32f2f", animation: "jpulse 1.1s ease-in-out infinite", flexShrink: 0 };
