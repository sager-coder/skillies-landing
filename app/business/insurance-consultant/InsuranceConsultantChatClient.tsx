...
/**
 * InsuranceConsultantChatClient — WhatsApp-styled demo chat for the
 * Insurance Consultant agent, with text AND voice.
 *
 * Text: type → /api/business/insurance-consultant streams a reply.
 * Voice: hold the mic → record → /transcribe (gpt-4o-transcribe,
 *   auto-detects Malayalam vs English) → /api/business/insurance-consultant
 *   in voice mode (agent replies in matching language, TTS-friendly
 *   script) → /tts speaks it back → the reply auto-plays as a voice note.
 *
 * So a customer can speak Malayalam or English and hear the answer back
 * in the same language — exactly like a real WhatsApp voice exchange.
 *
...
const STORAGE_KEY = "insurance-consultant.demo.v1";

const INTRO: Msg = {
  id: "intro",
  role: "assistant",
  content:
    "നമസ്കാരം 🙏 Insurance Consultant-ലേക്ക് സ്വാഗതം. എന്താണ് നോക്കുന്നത് — life cover (term), guaranteed savings, investment plan, retirement pension, അല്ലെങ്കിൽ child plan? (Type ചെയ്യാം, അല്ലെങ്കിൽ mic പിടിച്ച് സംസാരിക്കാം.)",
  ts: Date.now(),
};
...
export default function InsuranceConsultantChatClient() {
  ...
  const send = useCallback(async () => {
    ...
    abortRef.current = ac;
    try {
      const res = await fetch("/api/business/insurance-consultant", {
        method: "POST",
  ...
  const sendRecording = useCallback(async () => {
    ...
    // 2) Transcribe (gpt-4o-transcribe — reliable Malayalam/English
    //    detection + correct native script). Convert to 16kHz mono WAV first.
    let transcript = "";
    let lang = "";
    try {
      let uploadBlob = blob;
      try {
        ...
        // Decode/resample failed — upload the original and let the API try.
      }
      const fd = new FormData();
      fd.append("file", uploadBlob, "voice.wav");
      const tr = await fetch("/api/business/insurance-consultant/transcribe", {
        method: "POST",
        body: fd,
      });
      if (tr.ok) {
    ...
    let aiDur = 0;
    try {
      const tts = await fetch("/api/business/insurance-consultant/tts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={page}>
      <div style={frame}>
        <header style={header}>
          <div style={avatar}>IC</div>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={headerName}>Insurance Consultant</div>
      ...
      <style>{`
        ...
        @keyframes icdot { 0%,80%,100% { transform: scale(0.6); opacity: 0.4; } 40% { transform: scale(1); opacity: 1; } }
        @keyframes icpulse { 0%,100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.5; transform: scale(0.8); } }
...
// 16kHz mono WAV — a format the transcription API reliably accepts.
...
function Spinner() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" aria-hidden style={{ animation: "icspin 0.9s linear infinite" }}>
...
function Dot({ d }: { d: string }) {
  return (
    <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#90a4ae", display: "inline-block", animation: `icdot 1.2s ${d} infinite ease-in-out` }} />
  );
}
...
const recDot: React.CSSProperties = { width: 10, height: 10, borderRadius: "50%", background: "#d32f2f", animation: "icpulse 1.1s ease-in-out infinite", flexShrink: 0 };
